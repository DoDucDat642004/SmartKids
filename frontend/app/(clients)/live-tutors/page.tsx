...
// --- ANIMATION VARIANTS (Cấu hình hiệu ứng) ---
...
export default function LiveTutorPage() {
  ...
  const handleEnroll = async (classId: string) => {
    if (!confirm("Xác nhận đăng ký tham gia lớp học này?")) return;

    setRegisteringId(classId);
    try {
      await liveClassService.enrollClass(classId);
      router.push("/my-classes");
    } catch (error) {
      alert("Đăng ký thất bại. Lớp có thể đã đầy hoặc bạn đã tham gia rồi.");
    } finally {
...
function ClassCard({ data, onEnroll, isProcessing }: any) {
  ...
  return (
    <motion.div
      variants={itemVariants} // Sử dụng variant con để hưởng Stagger effect
      layout // Giúp card di chuyển mượt mà khi layout thay đổi
          ...
          <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
            <div className="w-10 h-10 rounded-full border border-slate-200 overflow-hidden relative">
              <img
                src={
                  data.tutorId?.avatar ||
                  `https://ui-avatars.com/api/?name=${data.tutorId?.fullName || "Tutor"}`
                }
                className="w-full h-full object-cover"
              />
            </div>
        ...
        <div className="mt-auto pt-4 flex items-center justify-between gap-4">
          <div>
            ...
            <span className="text-blue-600 font-black text-xl">
              {data.price ? `${data.price.toLocaleString()}đ` : "Miễn phí"}
            </span>
          </div>
          ...
          <motion.button
            ...
            className="flex-1 bg-slate-900 text-white px-4 py-3 rounded-xl font-bold text-sm hover:bg-blue-700 transition flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed shadow-lg shadow-slate-200"
          >
            {isProcessing ? (
              <Loader2 className="animate-spin" size={18} />
            ) : (
              <>
                Đăng ký <CheckCircle2 size={16} />
              </>
            )}
          </motion.button>
        </div>
